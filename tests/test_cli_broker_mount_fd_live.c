...
static int probe_brokered_mount_fd_move_mount(void) {
#if !defined(SYS_open_tree) || !defined(SYS_move_mount)
  return 0;
#else
  char root[] = "/tmp/landlockd-broker-mount-fd-probe-XXXXXX";
...
int main(int argc, char *argv[]) {
  char tempdir[] = "/tmp/landlockd-broker-mount-fd-live-XXXXXX";
  ...
  if (landlock_abi_version() == 0 || !landlockd_seccomp_probe_user_notif() ||
      !probe_brokered_mount_fd_move_mount()) {
    plan(SKIP_ALL,
         "Landlock, seccomp notify, or brokered open_tree plus move_mount is unavailable");
    done_testing();
  }
  ...
  direct_no_broker_argv[9] = "open-tree-move-read-cycle";
  ...
  direct_ro_read_argv[9] = "open-tree-move-read-cycle";
  ...
  direct_ro_write_argv[9] = "open-tree-move-append-denied-cycle";
  direct_ro_write_argv[10] = NULL;
  ok(run_landlockd(argv[1], direct_ro_write_argv, &status) == 0 &&
         WIFEXITED(status) && WEXITSTATUS(status) == 0 &&
         file_matches(source_ro_file, "ro"),
     "brokered read-only mount-fd paths do not permit writes");
  ...
  direct_rw_write_argv[9] = "open-tree-move-append-cycle";
  ...
  ok(run_landlockd(argv[1], direct_rw_write_argv, &status) == 0 &&
     ...
     "brokered mount-fd move_mount can modify the declared source when writable");
  ...
  direct_denied_argv[9] = "open-tree-move-read-cycle";
  direct_denied_argv[10] = NULL;
  ok(run_landlockd(argv[1], direct_denied_argv, &status) == 0 &&
         WIFEXITED(status) && WEXITSTATUS(status) == 17,
     "brokered mount-fd move_mount requests are denied on undeclared targets");
  ...
  ok(spawn_quiet(argv[1], serve_argv, &daemon_pid) == 0 &&
     ...
     "daemon starts for broker mount-fd runs");
  ...
  daemon_rw_write_argv[11] = "open-tree-move-append-cycle";
  daemon_rw_write_argv[12] = NULL;
  ok(run_landlockd(argv[1], daemon_rw_write_argv, &status) == 0 &&
         WIFEXITED(status) && WEXITSTATUS(status) == 0 &&
         file_matches(source_rw_file, "rw!"),
     "daemon runs also broker mount-fd requests inside the sandbox namespace");
  ...
  ok(run_landlockd(argv[1], stop_argv, &status) == 0 && WIFEXITED(status) &&
     ...
     "daemon stop exits successfully after broker mount-fd runs");

  ok(waitpid(daemon_pid, &daemon_status, 0) == daemon_pid &&
         WIFEXITED(daemon_status) && WEXITSTATUS(daemon_status) == 0,
     "daemon exits cleanly after broker mount-fd runs");

  ok(access(socket_path, F_OK) < 0 && errno == ENOENT,
     "daemon stop removes the control socket after broker mount-fd runs");
