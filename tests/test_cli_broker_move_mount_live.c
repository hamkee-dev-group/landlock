...
static int probe_brokered_move_mount(void) {
#if !defined(SYS_open_tree) || !defined(SYS_move_mount)
  return 0;
#else
  char root[] = "/tmp/landlockd-broker-move-mount-probe-XXXXXX";
  ...
  if (helper_pid == 0) {
    ...
    tree_fd = syscall(SYS_open_tree, AT_FDCWD, source_dir,
                      OPEN_TREE_CLONE | OPEN_TREE_CLOEXEC);
    if (tree_fd < 0 ||
        syscall(SYS_move_mount, tree_fd, "", AT_FDCWD, target_dir,
                MOVE_MOUNT_F_EMPTY_PATH) < 0 ||
        syscall(SYS_umount2, target_dir, 0) < 0) {
      if (tree_fd >= 0) {
        close(tree_fd);
      }
      close(user_fd);
...
int main(int argc, char *argv[]) {
  char tempdir[] = "/tmp/landlockd-broker-move-mount-live-XXXXXX";
  ...
  if (landlock_abi_version() == 0 || !landlockd_seccomp_probe_user_notif() ||
      !probe_brokered_move_mount()) {
    plan(SKIP_ALL,
         "Landlock, seccomp notify, or brokered move_mount is unavailable");
    done_testing();
  }
  ...
  if (write_policy(policy_without_bind, helper_dir, target_ro_dir, target_rw_dir,
    ...
    diag("cannot write broker move-mount policies: %s", strerror(errno));
    return 1;
  }
  ...
  direct_no_broker_argv[9] = "move-mount-read-cycle";
  ...
  ok(run_landlockd(argv[1], direct_no_broker_argv, &status) == 0 &&
     ...
     "without a broker bind rule, runtime move_mount requests fail closed");
  ...
  direct_ro_read_argv[9] = "move-mount-read-cycle";
  ...
  ok(run_landlockd(argv[1], direct_ro_read_argv, &status) == 0 &&
     ...
     "brokered move_mount requests succeed and remain ephemeral");
  ...
  direct_rw_write_argv[9] = "move-mount-append-cycle";
  ...
  ok(run_landlockd(argv[1], direct_rw_write_argv, &status) == 0 &&
     ...
     "brokered move_mount can modify the declared source when writable");
  ...
  direct_denied_argv[9] = "move-mount-read-cycle";
  direct_denied_argv[10] = NULL;
  ok(run_landlockd(argv[1], direct_denied_argv, &status) == 0 &&
         WIFEXITED(status) && WEXITSTATUS(status) == 17,
     "brokered move_mount requests are denied on undeclared targets");
  ...
  ok(spawn_quiet(argv[1], serve_argv, &daemon_pid) == 0 &&
     ...
     "daemon starts for broker move_mount runs");
  ...
  daemon_rw_write_argv[11] = "move-mount-append-cycle";
  daemon_rw_write_argv[12] = NULL;
  ok(run_landlockd(argv[1], daemon_rw_write_argv, &status) == 0 &&
         WIFEXITED(status) && WEXITSTATUS(status) == 0 &&
         file_matches(source_rw_file, "rw!"),
     "daemon runs also broker move_mount requests inside the sandbox namespace");
  ...
  ok(run_landlockd(argv[1], stop_argv, &status) == 0 && WIFEXITED(status) &&
     ...
     "daemon stop exits successfully after broker move_mount runs");

  ok(waitpid(daemon_pid, &daemon_status, 0) == daemon_pid &&
         WIFEXITED(daemon_status) && WEXITSTATUS(daemon_status) == 0,
     "daemon exits cleanly after broker move_mount runs");

  ok(access(socket_path, F_OK) < 0 && errno == ENOENT,
     "daemon stop removes the control socket after broker move_mount runs");
