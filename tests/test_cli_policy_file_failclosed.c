...
struct cli_state {
    ...
    int ir_init_call_count;
    int ir_reset_call_count;
    int ir_reset_saw_net_enabled;
    int fork_call_count;
...
{
    state.ir_reset_call_count++;
    if (ir != NULL) {
        if (ir->net_enabled) {
            state.ir_reset_saw_net_enabled = 1;
        }
        memset(ir, 0, sizeof(*ir));
    }
}
                               ...
                               FILE *err_stream)
{
    ...
    out_ir->net_enabled = 1;
    out_ir->net_handled_access = 0x3;
    return 0;
}
...
static int stderr_names_network_abi_requirement(const char *buf)
{
    return strstr(buf, "network") != NULL &&
           strstr(buf, "ABI") != NULL &&
           strstr(buf, "4") != NULL;
}
...
int main(void)
{
    ...
    ok(rc == 1, "run --policy-file with net policy on ABI 1 exits 1");
    ok(state.load_file_call_count == 1,
       "landlockd_policy_load_file is invoked exactly once");
    ok(stderr_names_network_abi_requirement(stderr_buf),
       "stderr diagnoses the ABI requirement for network policy");
    ok(state.ir_reset_saw_net_enabled == 1,
       "loaded IR still has net_enabled when reset is invoked");
    ok(state.ir_reset_call_count >= 1,
       "landlockd_policy_ir_reset is invoked before return");
    ...
    ok(state.apply_sandbox_call_count == 0 &&
       ...
       "landlockd_apply_sandbox and execvp are never called");
